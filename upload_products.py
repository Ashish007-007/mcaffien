...
upload_products.py
------------------
This script reads product data from an Excel file and creates/updates
products in a Shopify store using the Admin GraphQL API.

High-level behavior:
- Read `EXCEL_FILE` into a pandas DataFrame.
- Group rows by `handle` (each group represents one product).
- For each product group: check if product exists; create or update as
  needed; create/update variants; upload images and metafields; publish
  to the Online Store channel.

Major helper functions (documented inline):
- `run_graphql_query` - low-level request wrapper with basic rate-limit
  handling and error checks.
- `upload_image_to_shopify` / `add_images_to_product` - handle image
  uploads (local files or external URLs).
- `create_metafield` / `add_metafields_to_product` - handle metafields.
- `check_product_exists`, `create_product_with_variants`,
  `create_variant`, `update_variant` - core product/variant work.

Security note: this project currently stores the `ADMIN_API_TOKEN` in
`credentials.py` which is not secure. For production use, prefer
environment variables or a secrets manager.
"""

# Construct the GraphQL endpoint and HTTP headers used for all requests.
# `BASE_URL` targets the Admin GraphQL endpoint for the configured
# `API_VERSION`. `HEADERS` includes the access token required to
...
def run_graphql_query(query, variables):
    ...
    try:
        ...
        # If Shopify returns cost/throttle information in `extensions`,
        # inspect the throttle status and pause briefly when available
        # capacity is low to avoid being throttled.
